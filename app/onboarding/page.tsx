...
import { Btn, BrandMark, Field, TextInput, TextArea, Select } from "@/components/ui";
import { OBJETIVOS, PLAZOS, RUBROS } from "@/lib/utils";
...
export default function OnboardingPage() {
  ...
  const [data, setData] = useState({ nombre: "", rubro: "", descripcion: "", ubicacion: "", webUrl: "", instagramHandle: "", noWeb: false, noInstagram: false, otrosCanales: "", objetivo: "", objetivoCustom: "", magnitud: 20, plazoId: "3m", plazoCustomDate: "", presupuestoMarketing: "300", capacidadEjecucion: "Lo hago yo" });
  const set = (key: string, value: unknown) => setData((current) => ({ ...current, [key]: value }));
  const plazo = PLAZOS.find((item) => item.id === data.plazoId) || PLAZOS[1];
  const plazoDias = data.plazoId === "custom" && data.plazoCustomDate ? Math.max(1, Math.ceil((new Date(data.plazoCustomDate).getTime() - Date.now()) / 86400000)) : plazo.dias;
  const plazoLabel = data.plazoId === "custom" && data.plazoCustomDate ? `Hasta ${data.plazoCustomDate}` : plazo.label;
  ...
  const canContinue = step === 0 ? Boolean(data.nombre.trim() && data.rubro && data.ubicacion.trim()) : step === 1 ? instagramOk && websiteOk : Boolean(data.objetivo && (data.objetivo !== "Otro" || data.objetivoCustom.trim()));
  ...
  return <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
    ...
    <div style={{ flex: 1, display: "flex", justifyContent: "center", padding: "46px 24px" }}><div style={{ width: "100%", maxWidth: 580 }} className="shp-fadeup" key={step}>
      {step === 0 && <><h1 className="shp-display" style={{ fontSize: 28, marginBottom: 7 }}>Contanos lo esencial</h1><p style={{ color: COLORS.inkSoft, marginBottom: 26 }}>Con esto podemos empezar a buscar el negocio y sus canales.</p><Field label="Nombre"><TextInput value={data.nombre} onChange={(value) => set("nombre", value)} placeholder="Ej: AP Medicina Estética" /></Field><Field label="Rubro"><Select value={data.rubro} onChange={(value) => set("rubro", value)} options={RUBROS} placeholder="Elegí un rubro" /></Field><Field label="Ciudad o zona"><TextInput value={data.ubicacion} onChange={(value) => set("ubicacion", value)} placeholder="Ej: Palermo, Buenos Aires" /></Field><Field label="Descripción breve (opcional)" hint="Una frase sobre lo que ofrecés alcanza."><TextArea value={data.descripcion} onChange={(value) => set("descripcion", value)} placeholder="Ej: Tratamientos faciales y corporales con atención personalizada." rows={3} /></Field></>}
      {step === 1 && <><h1 className="shp-display" style={{ fontSize: 28, marginBottom: 7 }}>¿Dónde podemos encontrarte?</h1><p style={{ color: COLORS.inkSoft, marginBottom: 26 }}>Podés completar uno, ambos o ninguno. NUVRA buscará el resto automáticamente.</p><Field label="Instagram @usuario o URL" hint={!instagramOk ? "Ingresá un usuario o una URL de perfil válida." : "Usaremos este dato para identificar correctamente el perfil."}><TextInput value={data.instagramHandle} onChange={(value) => { set("instagramHandle", value); set("noInstagram", false); }} placeholder="@apmedicinaestetica" /></Field><button type="button" onClick={() => setData((current) => ({ ...current, noInstagram: !current.noInstagram, instagramHandle: !current.noInstagram ? "" : current.instagramHandle }))} style={{ border: 0, background: "none", color: COLORS.blueDeep, fontSize: 13, margin: "-10px 0 22px" }}>{data.noInstagram ? "Sí tengo Instagram" : "No tengo Instagram"}</button><Field label="Página web URL" hint={!websiteOk ? "Ingresá una dirección web válida." : "También puede quedar vacío."}><TextInput value={data.webUrl} onChange={(value) => { set("webUrl", value); set("noWeb", false); }} placeholder="apmedicina.com" /></Field><button type="button" onClick={() => setData((current) => ({ ...current, noWeb: !current.noWeb, webUrl: !current.noWeb ? "" : current.webUrl }))} style={{ border: 0, background: "none", color: COLORS.blueDeep, fontSize: 13, margin: "-10px 0 22px" }}>{data.noWeb ? "Sí tengo página web" : "No tengo página web"}</button><Field label="Otros canales o información (opcional)" hint="Escribilo con tus palabras."><TextArea value={data.otrosCanales} onChange={(value) => set("otrosCanales", value)} placeholder="Ej: Tenemos WhatsApp Business, local físico y recibimos turnos por teléfono." rows={3} /></Field></>}
      {step === 2 && <><h1 className="shp-display" style={{ fontSize: 28, marginBottom: 7 }}>¿Qué querés conseguir?</h1><p style={{ color: COLORS.inkSoft, marginBottom: 24 }}>NUVRA va a ordenar el análisis y las acciones alrededor de este objetivo.</p><ChoiceGrid options={OBJETIVOS} value={data.objetivo} onChange={(value) => set("objetivo", value)} />{data.objetivo === "Otro" && <div style={{ marginTop: 16 }}><Field label="Contanos el objetivo"><TextInput value={data.objetivoCustom} onChange={(value) => set("objetivoCustom", value)} /></Field></div>}<div style={{ marginTop: 24 }}><Field label="¿Cuánto te gustaría mejorar?"><div style={{ textAlign: "center", marginBottom: 8, fontWeight: 700, color: COLORS.blue }}>+{data.magnitud}%</div><input type="range" min={5} max={60} value={data.magnitud} onChange={(event) => set("magnitud", Number(event.target.value))} style={{ width: "100%", accentColor: COLORS.blue }} /></Field><Field label="¿En cuánto tiempo?"><ChoiceGrid options={PLAZOS.filter((item) => item.id !== "custom").map((item) => item.label)} value={plazoLabel} onChange={(label) => set("plazoId", PLAZOS.find((item) => item.label === label)?.id || "3m")} /></Field><Field label="Presupuesto aproximado"><ChoiceGrid options={BUDGETS.map((item) => item.label)} value={BUDGETS.find((item) => item.value === data.presupuestoMarketing)?.label || ""} onChange={(label) => set("presupuestoMarketing", BUDGETS.find((item) => item.label === label)?.value || "300")} /></Field><Field label="¿Quién puede llevar adelante las acciones?"><ChoiceGrid options={CAPACITIES} value={data.capacidadEjecucion} onChange={(value) => set("capacidadEjecucion", value)} /></Field></div>{error && <p style={{ color: COLORS.red, fontSize: 14, marginTop: 18 }}>{error}</p>}</>}
