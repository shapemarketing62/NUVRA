    ...
    <div className="action-list">
      {[...filtered].sort((a, b) => (a.order ?? Number.MAX_SAFE_INTEGER) - (b.order ?? Number.MAX_SAFE_INTEGER)).map((action, index) => <article className="action-item" key={action.id} style={{ opacity: action.done ? .72 : 1 }}>
        ...
        <div>
          ...
          {(action.details?.steps.length || action.evidence || action.inference || action.rationale || action.dependencies?.length || action.relatedConclusion) && <details className="action-plan"><summary>Ver plan de ejecución</summary><div className="action-plan-grid">{action.rationale && <div className="action-plan-wide"><strong>Por qué</strong><p>{simplifyTechnicalText(action.rationale)}</p></div>}{canonicalStrategy?.direction && <div className="action-plan-wide"><strong>Dirección del plan</strong><p>{simplifyTechnicalText(canonicalStrategy.direction)}</p></div>}{action.details?.audience && <div><strong>Para quién</strong><p>{simplifyTechnicalText(action.details.audience)}</p></div>}{action.details?.where && <div><strong>Dónde</strong><p>{simplifyTechnicalText(action.details.where)}</p></div>}{action.details?.steps.length ? <div className="action-plan-wide"><strong>Pasos</strong><ol>{action.details.steps.map((step) => <li key={step}>{simplifyTechnicalText(step)}</li>)}</ol></div> : null}{action.details && !action.details.experiment ? <div className="action-plan-wide experiment-panel"><strong>Cómo comprobar el resultado</strong><p><b>Duración:</b> {simplifyTechnicalText(action.details.timeframe || action.estimatedTime)}</p><p><b>Medición inicial:</b> {action.details.baseline ? simplifyTechnicalText(action.details.baseline) : "Todavía no está medida; registrala antes de empezar."}</p><p><b>Criterio de éxito:</b> {simplifyTechnicalText(action.details.successCriterion)}</p><p><b>Si funciona:</b> {simplifyTechnicalText(action.details.ifWorks)}</p><p><b>Si no funciona:</b> {simplifyTechnicalText(action.details.ifNot)}</p></div> : null}{action.details?.experiment && <div className="action-plan-wide experiment-panel"><strong>Qué busca comprobar la prueba</strong><p>{simplifyTechnicalText(action.details.experiment.hypothesis)}</p><p><b>Duración:</b> {simplifyTechnicalText(action.details.experiment.duration)}</p><p><b>Medición inicial:</b> {simplifyTechnicalText(action.details.experiment.baselineMetric)}</p><p><b>Criterio de éxito:</b> {simplifyTechnicalText(action.details.experiment.successCriteria)}</p><p><b>Si funciona:</b> {simplifyTechnicalText(action.details.experiment.ifWorks)}</p><p><b>Si no funciona:</b> {simplifyTechnicalText(action.details.experiment.ifNot)}</p></div>}{(action.details?.metric || action.indicatorToImprove) && <div><strong>Cómo medirlo</strong><p>{simplifyTechnicalText(action.details?.metric || action.indicatorToImprove || "")}</p></div>}{action.details?.estimatedCost && <div><strong>Costo estimado</strong><p>{simplifyTechnicalText(action.details.estimatedCost)}</p></div>}{action.dependencies?.length ? <div><strong>Antes de empezar</strong><ul>{action.dependencies.map((dependency) => <li key={dependency}>{simplifyTechnicalText(dependency)}</li>)}</ul></div> : null}{action.evidence && <div className="action-plan-wide"><strong>Información que sostiene esta acción</strong><p>{simplifyTechnicalText(action.evidence)}</p></div>}</div></details>}
