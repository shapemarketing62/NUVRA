...
function EvidenceLinks({ item }: { item: CompetitionCompetitorView }) {
  ...
  return <details className="supporting-evidence"><summary>Ver fuentes observadas ({item.evidence.length})</summary><div>{item.evidence.map((evidence, index) => evidence.url ? <a key={`${evidence.url}-${index}`} href={evidence.url} target="_blank" rel="noopener noreferrer">{evidence.sourceType}: {evidence.label}</a> : <span key={`${evidence.label}-${index}`}>{evidence.sourceType}: {evidence.label}</span>)}</div></details>;
}

function ComparableBusiness({ item }: { item: CompetitionCompetitorView }) {
  return <Card><div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 18 }}><div><h2 className="section-title">{item.name}</h2>{item.location && <p style={{ fontSize: 12, color: COLORS.inkSoft, marginTop: 5 }}>{item.location}</p>}</div><StatusBadge tone="success">Comparable</StatusBadge></div>
...
export default function CompetenciaPage() {
  ...
  return <div className="page-container competition-v3"><PageHeader eyebrow="Mercado" title="Competencia" subtitle="Con quién tiene sentido compararte y qué diferencias pudimos sostener con información verificable." action={<StatusBadge tone={competition.comparable.length ? "success" : "neutral"}>{competition.comparable.length} comparables</StatusBadge>} />
    {competition.context && <p style={{ color: COLORS.inkSoft, fontSize: 13, lineHeight: 1.6, marginBottom: 22 }}>{competition.context}</p>}

    {competition.comparable.length ? <section className="competition-list"><h2 className="section-title" style={{ marginBottom: 14 }}>Negocios comparables</h2><div className="stack">{competition.comparable.map((item) => <ComparableBusiness key={item.name} item={item} />)}</div></section> : <EmptyState title="No encontramos suficientes negocios comparables" description="Los candidatos encontrados no tienen evidencia suficiente de categoría, modelo y ubicación para hacer una comparación confiable." />}

    {competition.probable.length > 0 && <section style={{ marginTop: 32 }}><h2 className="section-title">Posibles comparables por confirmar</h2><p className="section-description">Pueden ser relevantes, pero todavía falta evidencia para compararlos en las mismas condiciones.</p><div className="stack" style={{ marginTop: 14 }}>{competition.probable.map((item) => <Card key={item.name}><div style={{ display: "flex", justifyContent: "space-between", gap: 16 }}><div><h3 className="section-title" style={{ fontSize: 16 }}>{item.name}</h3>{item.location && <p style={{ color: COLORS.inkSoft, fontSize: 12, marginTop: 5 }}>{item.location}</p>}</div><StatusBadge tone="warning">Por confirmar</StatusBadge></div>{item.whyComparable.length > 0 && <p style={{ color: COLORS.inkSoft, fontSize: 13, lineHeight: 1.55, marginTop: 14 }}>{item.whyComparable.join(" · ")}</p>}<EvidenceLinks item={item} /></Card>)}</div></section>}
  </div>;
}
