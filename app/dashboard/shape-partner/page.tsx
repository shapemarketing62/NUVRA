...
export default function PartnerPage() {
  const { planTier, internalAccess, loading, error } = useDashboardData();
  if (loading) return <PageSkeleton />;
  if (error) return <ErrorState message={error} />;
  if (!hasEntitlement(planTier, "workspace.overview", internalAccess)) return <div className="page-container"><div className="page-eyebrow">Para agencias y consultores</div><h1 className="page-title" style={{ marginBottom: 10 }}>NUVRA Partner</h1><p className="page-subtitle" style={{ marginBottom: 28 }}>Un espacio preparado para administrar clientes, negocios y equipo sin mezclar información.</p><UpgradePanel feature="workspace.overview" /></div>;
