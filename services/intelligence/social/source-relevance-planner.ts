...
export class SourceRelevancePlanner {
  static plan(target: SocialBusinessTarget): PlannedSocialSource[] {
    const text = normalize(`${target.industry} ${target.customerType || ""} ${target.objective || ""}`);
    ...
    const local = /cafe|restaurante|gastronom|barber|peluquer|clinica|salud|gimnas|local|turismo|hotel/.test(text);
    const ecommerce = /ecommerce|tienda online|producto|retail|venta online/.test(text);
    return platforms.map((platform) => {
      ...
      if (b2b && ["linkedin", "x", "youtube"].includes(platform)) { score += platform === "linkedin" ? .65 : .35; reasons.push("aporta autoridad o conversación B2B"); }
